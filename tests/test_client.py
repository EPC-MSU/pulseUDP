...
def _client_replying(reply_version, probe_log=None):
    ...
    Models a reachable controller that stamps its reply with ``reply_version``
...
def test_negotiation_fixes_v01_in_one_round_trip():
    # v0.1 controller answers the v1.0 probe and reveals v0.1; client downgrades.
    events = []
...
def test_negotiation_no_reply_times_out():
    # Unreachable endpoint: the controller never answers (no version branch).
    c = UdpClient("127.0.0.1")
    c._sock = _FakeSock()
