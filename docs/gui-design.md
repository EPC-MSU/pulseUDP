...
- **Per-version wire behavior:** inbound datagrams are judged by their **own** version field.
  Under v1.0 the CRC is validated over Magic..Reserved and a mismatch drops the datagram with a
  `crc` log row, while telemetry sequence loss logs a `seq_gap` row. Under v0.1 the sequence and
...
reassembly (RFC §5.7) — the v1.0 single-datagram path is implemented and the CRC is validated
